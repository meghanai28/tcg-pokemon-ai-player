...
class NumpyNet:
    ...
    def forward(self, kind, card, scal, mask, ctx_id, stype_id):
        ...
        option = (x @ w["option_head.weight"].T + w["option_head.bias"]).squeeze(-1)
        option = np.where(mask < 0.5, -1e9, option)
        count = x[:, 0, :] @ w["count_head.weight"].T + w["count_head.bias"]
        h = _gelu(x[:, 0, :] @ w["value_fc1.weight"].T + w["value_fc1.bias"])
        value = np.tanh(
            (h @ w["value_fc2.weight"].T + w["value_fc2.bias"]).squeeze(-1)
        )
