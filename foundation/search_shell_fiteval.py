...
def _evaluate(state, me):
    ...
    Shipped AUC 0.6567, this 0.7400, on 124,054 positions.
    ...
    z = (2.669761 * (opp_prize - my_prize) / 6.0
         + -0.249818 * (len(my_board) - len(opp_board))
         + 0.315766 * (mh - oh)
         + 0.117029 * (men - oen)
         + 0.013311 * (mat - oat)
         + 0.06856 * (ms1 - os1)
         + -0.200077 * (ms2 - os2)
         + 0.094004 * min(hand_n, 16)
         + -0.093279 * min(opl.get("handCount", 0), 16)
         + -2.440496 * (1.0 if mypl.get("deckCount", 1) == 0 else 0.0)
         + -0.308896 * (1.0 if 0 < mypl.get("deckCount", 99) <= 2 else 0.0)
         + 2.489709 * (1.0 if opl.get("deckCount", 1) == 0 else 0.0)
         + 2.535709 * status
         + -0.002643) / 1.0
    if z >= 30.0:
        return 0.97
