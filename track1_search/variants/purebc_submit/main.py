...
def agent(obs):
    ...
    try:
        ...
        # PURE-POLICY submission (scaled BC, dim192/6L): one network forward,
        # argmax over legal options, NO search. Reuses _net_scores, the exact
        # inference path the search agent uses for root priors. Search below is
        # kept only as a fallback if the network fails to load/score.
        try:
            _sc = [_option_score(o, sel) for o in opts]
            _learned = _net_scores({"current": cur, "select": sel}, me, sel, opts, _sc)
            if _learned is not None:
                _order = sorted(range(n), key=lambda i: -_learned[i])
                _act = _validate(_order[:kmax], sel)
                if _act is not None:
                    return _act
        except Exception as _exc:
            _dbg("pure-policy failed, falling back:", repr(_exc))
